...
export function RequestRow({ request, staffById, now, onCancel, onToggleFavorite }: RequestRowProps) {
  ...
  return (
    <article className="grid grid-cols-[auto_1fr] gap-x-3.5 gap-y-3 rounded-[10px] border border-border bg-card p-4">
      <span
        className="col-start-1 row-start-1 flex size-[42px] shrink-0 items-center justify-center rounded-[9px]"
      ...
      <div className="col-start-2 row-start-1 flex min-w-0 items-start justify-between gap-3">
        <div className="min-w-0 flex flex-col gap-1">
          ...
          <p className="flex items-center gap-1.5 text-xs leading-4 text-muted-gray">
            <Icon name={assign.icon} className="size-[15px] shrink-0 text-electric" aria-hidden />
            {assign.text}
          </p>
        </div>
      ...
      {doneIn && (
        <p className="col-start-2 text-xs font-medium text-green-700">{doneIn}</p>
      )}
      {onCancel && (
